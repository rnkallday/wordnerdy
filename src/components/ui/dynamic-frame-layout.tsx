...
function FrameComponent({
  ...
  return (
    <div
      className={`relative ${className} ${content.link ? 'cursor-pointer hover:opacity-90 transition-opacity' : ''}`}
      style={{
        width,
        height,
      ...
      onClick={handleClick}
      role={content.link ? "link" : undefined}
      tabIndex={content.link ? 0 : undefined}
      onKeyDown={(e) => {
        if (content.link && (e.key === 'Enter' || e.key === ' ')) {
          handleClick()
        }
      }}
    >
